'use client'

import { motion } from 'framer-motion'

const ROLE_META: Record<string, { label: string; color: string; bg: string; bar: string }> = {
  owner:   { label: 'Owner',   color: 'text-red-400',    bg: 'bg-red-400/10 border-red-400/30',       bar: 'from-red-500/50 via-red-400 to-red-500/50' },
  admin:   { label: 'Admin',   color: 'text-orange-400', bg: 'bg-orange-400/10 border-orange-400/30', bar: 'from-orange-500/50 via-orange-400 to-orange-500/50' },
  premium: { label: 'Premium', color: 'text-purple-400', bg: 'bg-purple-400/10 border-purple-400/30', bar: 'from-purple-500/50 via-purple-400 to-purple-500/50' },
  vip:     { label: 'VIP',     color: 'text-yellow-400', bg: 'bg-yellow-400/10 border-yellow-400/30', bar: 'from-yellow-500/50 via-yellow-400 to-yellow-500/50' },
  player:  { label: 'Player',  color: 'text-[var(--muted)]', bg: 'bg-[var(--surface-2)] border-[var(--border)]', bar: 'from-[#4ade80]/40 via-[#4ade80] to-[#4ade80]/40' },
}

type Profile = {
  nickname: string
  display_name: string | null
  avatar_url: string | null
  role: string
  play_time_hours: number
  total_kills: number
  total_deaths: number
  balance: number
  minecraft_name: string | null
  created_at: string
}

const STATS = (p: Profile) => [
  {
    label: 'Час гри',
    value: `${p.play_time_hours ?? 0}г`,
    icon: (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
      </svg>
    ),
    color: 'text-blue-400',
    bg: 'bg-blue-400/8',
  },
  {
    label: 'Вбивства',
    value: p.total_kills ?? 0,
    icon: (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M14.5 10c-.83 0-1.5-.67-1.5-1.5v-5c0-.83.67-1.5 1.5-1.5s1.5.67 1.5 1.5v5c0 .83-.67 1.5-1.5 1.5z"/>
        <path d="M20.5 10H19V8.5c0-.83.67-1.5 1.5-1.5s1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/>
        <path d="M9.5 14.5v-5c0-.83-.67-1.5-1.5-1.5S6.5 8.67 6.5 9.5v5"/>
        <path d="M3.5 10H5v-1.5C5 7.67 4.33 7 3.5 7S2 7.67 2 8.5 2.67 10 3.5 10z"/>
        <path d="M11.5 15H6l-2 6h14l-2-6h-4.5z"/>
      </svg>
    ),
    color: 'text-red-400',
    bg: 'bg-red-400/8',
  },
  {
    label: 'Смерті',
    value: p.total_deaths ?? 0,
    icon: (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M12 2a9 9 0 0 1 9 9c0 3.18-1.65 5.97-4.13 7.59L16 21H8l-.87-2.41A9.01 9.01 0 0 1 3 11a9 9 0 0 1 9-9z"/>
      </svg>
    ),
    color: 'text-[var(--muted)]',
    bg: 'bg-[var(--surface-2)]',
  },
  {
    label: 'Баланс',
    value: `${Number(p.balance ?? 0).toFixed(2)} ₴`,
    icon: (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <line x1="12" y1="1" x2="12" y2="23"/>
        <path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>
      </svg>
    ),
    color: 'text-[#4ade80]',
    bg: 'bg-[#4ade80]/8',
  },
]

export function ProfileHeader({ profile, email }: { profile: Profile; email: string }) {
  const role  = ROLE_META[profile.role] ?? ROLE_META.player
  const stats = STATS(profile)
  const joinDate = new Date(profile.created_at).toLocaleDateString('uk-UA', {
    day: 'numeric', month: 'long', year: 'numeric',
  })
  const displayName = profile.display_name || profile.nickname

  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}
      className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl overflow-hidden"
    >
      {/* Role-coloured accent bar */}
      <div className={`h-1 w-full bg-gradient-to-r ${role.bar}`} />

      <div className="p-5 sm:p-6">
        {/* Avatar + name row */}
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-[var(--surface-2)] border-2 border-[var(--border)] flex items-center justify-center overflow-hidden shrink-0">
            {profile.avatar_url
              ? <img src={profile.avatar_url} alt="avatar" className="w-full h-full object-cover" />
              : <span className="text-2xl sm:text-3xl font-black text-[var(--muted)]">{displayName.charAt(0).toUpperCase()}</span>
            }
          </div>

          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-lg sm:text-xl font-bold text-[var(--foreground)] truncate">{displayName}</h1>
              <span className={`inline-flex items-center text-xs font-bold px-2.5 py-1 rounded-lg border ${role.bg} ${role.color}`}>
                {role.label}
              </span>
            </div>
            <p className="text-[var(--muted)] text-sm mt-0.5">@{profile.nickname}</p>

            {/* Meta info */}
            <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs text-[var(--muted)]">
              <span className="flex items-center gap-1.5">
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
                  <polyline points="22,6 12,13 2,6"/>
                </svg>
                {email}
              </span>
              {profile.minecraft_name && (
                <span className="flex items-center gap-1.5">
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <rect x="2" y="3" width="20" height="14" rx="2"/>
                    <line x1="8" y1="21" x2="16" y2="21"/>
                    <line x1="12" y1="17" x2="12" y2="21"/>
                  </svg>
                  {profile.minecraft_name}
                </span>
              )}
              <span className="flex items-center gap-1.5">
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <rect x="3" y="4" width="18" height="18" rx="2" ry="2"/>
                  <line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/>
                  <line x1="3" y1="10" x2="21" y2="10"/>
                </svg>
                З нами від {joinDate}
              </span>
            </div>
          </div>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mt-5">
          {stats.map(({ icon, label, value, color, bg }, i) => (
            <motion.div
              key={label}
              initial={{ opacity: 0, y: 6 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.1 + i * 0.05, duration: 0.3 }}
              className={`${bg} rounded-xl p-3 border border-[var(--border)]`}
            >
              <div className={`flex items-center gap-1.5 mb-1.5 ${color}`}>
                {icon}
                <span className="text-[11px] text-[var(--muted)] font-medium">{label}</span>
              </div>
              <p className="text-[var(--foreground)] font-bold text-sm">{value}</p>
            </motion.div>
          ))}
        </div>
      </div>
    </motion.div>
  )
}
